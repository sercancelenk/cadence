...
## 9. Native notifications & reminders

Immediate notifications (legacy path):
...
**Scheduled reminders** live in `electron/reminder/` and run in the **main process**:

| Piece | Role |
|---|---|
| `index.cjs` | Reconcile AppData → in-process timer + OS adapter on every save |
| `adapters/macos.cjs` | Swift helper + long-lived **agent** for quit-after-fire + click deep links |
| `adapters/windows.cjs` | C# `ScheduledToastNotification` helper |
| `linuxBackground.cjs` | Tray + launch-at-login (no OS schedule on Linux) |
| `preload.cjs` | `syncReminders`, `cancelReminderSlots`, `reminderSyncStatus`, `onReminderEvent` |

Flow:

1. Renderer saves workspace → `data:save` → `syncRemindersFromAppData(payload)`.
2. Main collects future slots, sets one `setTimeout`, and reconciles OS notifications.
3. On fire: persist **before** showing notification, then cancel matching OS slot.
4. Notification click → `cadence://todo/:id` or `cadence://item/:id` → renderer route.

The renderer **does not** run `useReminderWatcher` on Electron (main owns scheduling). PWA uses a service worker when `Notification` supports `showTrigger` (Chrome).

**Build helpers:** `npm run build:reminder-helper` (macOS Swift binary), `npm run build:reminder-helper-win` (Windows C# toast helper). Linux uses in-process scheduling + optional tray (`linuxBackground.cjs`); no OS-level schedule.

**Platform matrix**

| Platform | OS schedule | Click deep link | Notes |
|---|---|---|---|
| macOS | Swift helper + agent | `cadence://todo/:id`, `cadence://item/:id` | Agent restarts on crash |
| Windows | C# ScheduledToast | Same scheme | Helper bundled in release CI |
| Linux | None (in-process only) | Same scheme | Tray + launch-at-login |
| PWA | SW `showTrigger` | In-app route via `notificationclick` | Fallback watcher when triggers unsupported |

Manual QA checklist and deferred items (systemd user unit, E2E reminder tests) are tracked in [`HEALTH-CHECK-AND-ROADMAP.md`](HEALTH-CHECK-AND-ROADMAP.md) Phase E.
