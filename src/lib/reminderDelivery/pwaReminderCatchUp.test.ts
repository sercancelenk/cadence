...
describe('parseReminderNotificationTag', () => {
  ...
  it('rejects unknown tags', () => {
    ...
    expect(parseReminderNotificationTag('not-cadence:item|2026-01-01T00:00:00.000Z')).toBeNull();
  });
...
describe('collectPwaDeliveredSlotKeys', () => {
  ...
  it('returns empty when notification permission is not granted', async () => {
    mockEnv({
      permission: 'denied',
      notifications: [{ tag: 'cadence:item-1|2020-01-01T10:00:00.000Z' }],
    });
    await expect(collectPwaDeliveredSlotKeys(Date.parse('2025-06-01T00:00:00.000Z'))).resolves.toEqual([]);
  });
  ...
  it('returns empty when service worker is not ready', async () => {
    ...
    const readyPromise = Promise.reject(new Error('no sw'));
    const catchSpy = vi.spyOn(readyPromise, 'catch');
    Object.defineProperty(globalThis, 'navigator', {
      configurable: true,
      value: {
        serviceWorker: { ready: readyPromise },
      },
    });
    await expect(collectPwaDeliveredSlotKeys()).resolves.toEqual([]);
  ...
  it('collects slot keys when remindAt equals nowMs exactly', async () => {
    const atNow = '2025-06-01T00:00:00.000Z';
    mockEnv({
      notifications: [{ tag: `cadence:item-now|${atNow}` }],
    });
    const keys = await collectPwaDeliveredSlotKeys(Date.parse(atNow));
    expect(keys).toEqual([`item-now\u0001${atNow}`]);
  });

  it('returns empty when Notification is missing from window', async () => {
    mockEnv({ notifications: [{ tag: 'cadence:item-1|2020-01-01T10:00:00.000Z' }] });
    Reflect.deleteProperty(globalThis, 'Notification');
    await expect(collectPwaDeliveredSlotKeys()).resolves.toEqual([]);
  });

  it('returns empty when service worker ready rejects without throwing', async () => {
    mockEnv({ ready: false, notifications: [{ tag: 'cadence:item-1|2020-01-01T10:00:00.000Z' }] });
    await expect(collectPwaDeliveredSlotKeys(Date.parse('2025-06-01T00:00:00.000Z'))).resolves.toEqual([]);
  });
});
